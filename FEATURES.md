...
מעודכן ל־2026-06-14. תכלס היא מחברת אישית חכמה בעברית, עם אפליקציית PWA עצמאית
כערוץ הראשי ועם בוט Telegram כערוץ אופציונלי נוסף. כל היכולות זמינות לכל משתמש
ללא תוכניות תשלום או הגבלת פיצ'רים. חברים יכולים להצטרף דרך הזמנות.
כל הממשק בעברית, והמזהים בקוד באנגלית.
...
## אפליקציית PWA עצמאית

- כניסה עם Supabase Auth ללא תלות ב־Telegram, עם אפשרות לקשר פרופיל Telegram קיים.
- הזנה חופשית בטקסט, בהקלטה קולית או בקובץ, דרך אותו נתב כוונות של הבוט.
- התראות Push לתזכורות ולסוכנים גם כשהאפליקציה סגורה.
- מסכים מלאים ליומן, תזכורות, פרויקטים, זיכרונות, משימות, רשימות ובריאות.
- חיפוש מאוחד בזיכרונות, רשימות ומשימות, לצד חיפוש ב־Google Drive.
- העלאת תמונה לזיכרון עם OCR. חילוץ אוטומטי של משימות ופריטי רשימה קיים
  כיכולת ניסיונית.
- נעיצת זיכרונות ללוח אישי, תצלומי מצב, פעילות אחרונה וציר זמן נרטיבי.
- ניהול סוכנים פרואקטיביים ושיתוף פריטים עם חברים שאינם חייבים חשבון Telegram.
- התקנה למסך הבית, Web Share Target וממשק RTL מותאם לנייד.
...
45. **`/extract` על תמונה (ניסיוני)** — מנסה לחלץ פעולות (משימות/פריטים)
    וליצור אותן אוטומטית — Image-to-Action.
