...
## המסלול המומלץ: אשף ההתקנה

ב-Windows אין צורך לעבור ידנית על כל המדריך:
...
האשף פועל רק על `127.0.0.1`, שומר התקדמות ומציג יומן ללא סודות. הוא:

1. בודק ומציע התקנה של כלי הפיתוח.
2. אוסף את פרטי Supabase ומחיל migrations.
3. בונה ומפרסם את ה-PWA ב-Vercel.
4. יוצר סודות פנימיים ומפתחות Push.
5. בודק מפתחות AI בהתקנה מלאה.
6. מעלה סודות ופורס Edge Functions.
7. מדריך בהגדרת Google עם כתובות Redirect מוכנות.
8. בודק את ה-PWA, את Supabase Auth ואת הגנת ה-API.

### פעולות ידניות שהאשף אינו עוקף

- יצירת חשבונות ופרויקטים ב-Supabase, Vercel ו-Google Cloud.
- אישור כניסה כאשר CLI פותח דפדפן.
- הגדרת Site URL ו-Redirect URLs ב-Supabase Auth.
- הרצת `supabase/.temp/self-host-vault.sql` ב-SQL Editor ומחיקתו.
- הפעלת APIs, הגדרת Audience ויצירת OAuth Client ב-Google Cloud.
- הפעלת Google Provider ב-Supabase Auth.

כל פעולה מוצגת בזמן המתאים עם קישור ישיר וערך מוכן להעתקה.

המשך המדריך הוא המסלול הידני ופתרון התקלות.

## 1. דרישות למסלול הידני
